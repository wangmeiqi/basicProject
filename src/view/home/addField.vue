	...
	<div class="addFieldBox">
		<div class="addFieldHeader">
			...
			<div class="addFieldHeaderRight">
				<el-button @click="saveMTBol=true" size="small" type="primary">保存</el-button><br />
					...
					<draggable v-model="myArray" :options="{group:{name: 'field',pull: false,put: true},animation:150}">
						<div class="itemAddField" v-for="(element,index) in myArray" :key="element.id">
							<div @mouseover="Fieldindex=index">
								...
								<span>待添加</span>
								<div class="itemAddFieldClose"><i v-show="element.colseBol" @click="transfer(index)" class="el-icon-circle-close"></i></div>
							</div>
						</div>
					</draggable>
				</div>
			...
			<div class="rights">
				...
				<draggable class="right" v-model="newField" :options="{group:{name: 'field',pull: true,put: false}}">
					<div class="itemAddField" v-for="(item,index) in newField" :key="index" @mouseover="item.btnBol=true" @mouseleave="item.btnBol=false">
						...
						<div v-if="item.btnBol" class="iconbtn">
							<i v-if="item.default_field != 3" @click="openEdit(index,1)" class="el-icon-arrow-down"></i>
						</div>
				...
				<div class="right defaults">
					...
					<div class="itemAddField" v-for="(item,index) in defaults" :key="index" @mouseover="item.btnBol=true" @mouseleave="item.btnBol=false">
						...
						<div>{{item.name}}</div>
						<div v-if="item.btnBol" class="iconbtn">
							<i @click="openEdit(index,2)" class="el-icon-arrow-down"></i>
						</div>
						<div class="SH" v-show="item.SHBol">
							<div class="SHTitle">
								<div></div>
				...
				<el-row>
					<el-col :span="24">
						<div class="field">类型</div>
						 <el-select v-model="FieldMessage.type" placeholder="请选择"  style="width: 100%;">
						    <el-option
		...
		<el-dialog :append-to-body="toBody" title="保存任务类型" :visible.sync="saveMTBol" width="400px" center>
			<span class="saveMTBox">
				<el-row>
					<el-col :span="24">
						<h2>设置任务类型</h2>
						<el-input :maxlength="20" v-model="taskTypeName" placeholder="设置任务类型，最多20个字，如需求"></el-input>
		...
		<el-dialog :append-to-body="toBody" title="查看详情/编辑" :visible.sync="compileBol" width="400px" center>
			<span class="addFieldMTBox">
				...
				<el-row>
					<el-col :span="24">
						<div class="field">类型</div>
						 <el-select disabled v-model="compileFieldMessage.type" placeholder="请选择" style="width: 100%;">
						    <el-option
						      v-for="item in selectList"
						      :key="item.value"
						      :label="item.label"
...
<script>
	...
	export default {
		...
		methods: {
			openEdit(index,flag){
				let oldIndex = this.newField.findIndex(el => {
					return el.SHBol == true
				})
				let oldIndex2 = this.defaults.findIndex(el => {
					return el.SHBol == true
				})
				this.newField[oldIndex == -1 ? 0:oldIndex].SHBol = false
				this.defaults[oldIndex2 == -1 ? 0:oldIndex2].SHBol = false
				if(flag == 1){
					this.newField[index].SHBol = !this.newField[index].SHBol
				}else {
					this.defaults[index].SHBol = !this.defaults[index].SHBol
				}
          ...
          typeId: 0
					...
					if(res.data.data) {
						API.post('/taskType/saveTaskType', {
							name: this.taskTypeName,
							projectId: sessionStorage.getItem('projectId'),
			...
			getField() {
				API.post('/taskType/GetField', {
					...
					res.data.data.map(el => {
						...
						if(el.default_field == 1) {

							this.myArray.push(el)
						} else if(el.default_field == 2) {
							this.newField.push(el)
						} else if(el.default_field == 3){
							this.newField.push(el)
						}
