<template>
	<div>
		<div class="corporateAuthority-head">
			<p>企业权限</p>
			<a class="addBox" v-show="newZdyjsFlage" @click="addCustomRole" v-if="addFlage">添加自定义角色</a>
			<a class="addBox defAdd" v-show="newZdyjsFlage" v-if="defAddFlage">添加自定义角色</a>
		</div>
			...
			<div class="width-limited">
				...
				<div class="left-wrap">
					...
					<div class="left-wrap-box">
						<template v-for="(val,index) in projectList" v-if='index<=3'>
							<div :class="index == 0?'group-gap-active group-gap active flags'+index:'group-gap-active group-gap flags'+index" v-show="val.id == index+1">{{val.name}}</div>
							<div :class="index == 0?'flageActive active flags'+index:'flageActive flags'+index">
				...
				<div class="right-wrap">
					<div class="right-wrap-list" v-for="(item,indexs) in roleList">
						<template v-if="indexs >2">
							<div class="right-wrap-head right-wrap-head-edit" @click="clickFlags && editRoleFun(indexs)" v-if="item.roleName != ''">
								...
								<div class="editBox" v-if="editRoleFlage == indexs">
									...
									<p class="edit" v-if="XGZdyjsFlage" @click="editCostomRoleFun(item.roleId)"><img src="../../assets/images/qianbi.png" />编辑角色</p>
									<p class="del" v-if="delZdyjsFlage" @click="delCostomRoleFun(item.roleId)"><img src="../../assets/images/newlj.png" />删除角色</p>
								</div>
							</div>
						...
						<div class="right-wrap-box">
							<template v-for="(val,index) in roleList[indexs].enterprisePermissionList" v-if='index<=3'>
								<div :class="'group-gap flags'+index" v-if="val.id == index+1">
									<img v-if="val.status == 1" src="../../assets/images/xzicoon.png" />
									<template v-if="val.status == 4">
										<span class="defImg active" :data-id='val.id' v-if="val.id>4?1:0"></span>
									</template>
									<template v-if="val.status == 3">
										<span class="defImg" :data-id='val.id' v-if="val.id>4?1:0"></span>
									</template>
								</div>
								<div :class="index == 0?'flageActive active flags'+index:'flageActive flags'+index">
									<template v-for="val in roleList[indexs].enterprisePermissionList">
...
<script>
	...
	export default {
		...
		methods: {
			...
			//查询企业下所有的角色和权限
			selectRoleListFun() {
				API.get('/enterprisePermission/queryEnterprisePermissionAll', {
					eid: sessionStorage.getItem('enterpriseId') //企业ID
				}).then(res => {
					console.log('查询企业下所有的角色和权限', res.data.data)
					this.roleList = res.data.data
					this.projectList = res.data.data[0].enterprisePermissionList
			...
			jqShowHideFun() {
				for(let i = 0; i < 4; i++) {
					$('.flags' + i).bind('click')
					$(document).on('click', '.group-gap-active.flags' + i, function() {
						$('.flags' + i).addClass('active')
					})
					$(document).on('click', '.group-gap-active.active.flags' + i, function() {
			...
			addCustomRole() {
				...
				this.roleNamesVal = ''
				this.defAddFun()
				API.get('/enterprisePermission/toAddEnterprisePermissionRole', {
					eid: sessionStorage.getItem('enterpriseId') //企业ID
				}).then(res => {
					console.log('添加自定义角色', res.data.data)
					this.customRole = {
						enterprisePermissionList: [],
						projectPermissionList: null,
						roleId: this.num++,
						roleName: ""
					}
					this.customRole.enterprisePermissionList = res.data.data
			...
			saveFun() {
				...
				for(let i = 0; i < this.customRole.enterprisePermissionList.length; i++) {
				...
				API.get('/enterprisePermission/addEnterprisePermissionRole', {
					roleName: roleNames,
					eid: sessionStorage.getItem('enterpriseId'), //企业ID
			...
			delCostomRoleFun(roleIds) { //roleIds为角色id
				this.$confirm('此操作将永久删除该角色, 是否继续?', '提示', {
					...
					API.get('/enterprisePermission/deleteEnterprisePermissionRole', {
						roleId: roleIds,
						eid: sessionStorage.getItem('enterpriseId') //企业ID
					}).then(res => {
						console.log('删除企业自定义角色', res.data.message)
						if(res.data.message == 'SUCCESS') {
							//添加成功后 执行查询事件
							this.selectRoleListFun()
						}
			...
			editCostomRoleFun(roleIds) { //roleIds为角色id
				...
				API.get('/enterprisePermission/findByRoleId', {
					roleId: roleIds,
				}).then(res => {
					console.log('编辑企业自定义角色', res.data.data)
			...
			editSaveFun() {
				...
				let dataId = '' //选中的id
				let roleNames = this.roleNamesVal
				for(let i = 0; i < this.roleList[this.roleSub].enterprisePermissionList.length; i++) {
					if($('span.defImg.active').eq(i).attr('data-id') != undefined) {
						dataId = dataId + $('span.defImg.active').eq(i).attr('data-id') + ','
					}
				}
				...
				API.get('/enterprisePermission/updateEnterprisePermissionRole', {
					id: this.roleIds,
					roleName: roleNames,
					eid: sessionStorage.getItem('enterpriseId'), //企业ID
					ids: dataId.substr(0, dataId.length - 1)
				}).then(res => {
					console.log('编辑企业自定义角色保存按钮事件', res.data.message)
					if(res.data.message == 'SUCCESS') {
						//消息提示框
						this.$notify({
							title: '成功',
			...
			.left-wrap {
				...
				.left-wrap-box {
					...
					.group-gap {
						cursor: pointer;
						padding-left: 20px;
						background: url(../../assets/images/xiajiantou.png) no-repeat 150px center;
					}
					.group-gap:hover {
						background: url(../../assets/images/xiajiantou.png) no-repeat 150px center #eee;
					}
					.permission-item {
						padding-left: 40px;
					}
