	...
	<div @click.stop="shows=shows">
		...
		<ul class="participant">
			<li v-for="(item,index) in list">
				...
				<i @click="delUser(item.id,index)" class="iconfont icon-guanbi close" v-if="removeParticipantFlage"></i>
			</li>
			<i @click="getStyles($event)" class="iconfont icon-tianjia1 addparticipant" v-if="addParticipantFlage"></i>
		</ul>
		<div>
			<memberlist @allmsg="getUsersMsg" @send="getUserMsg" :principalId="principalId" type="checked" :styles="styles" :ids="ids" :show="shows" :taskId="taskId"></memberlist>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods:{
			getStyles(el){				//给邀请成员组件定位
				...
				this.styles = 'top:'+ el.clientY +'px;left:'+ el.clientX +'px'
			},
